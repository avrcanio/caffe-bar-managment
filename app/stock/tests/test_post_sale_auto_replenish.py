...
from stock.models import StockAccountingConfig, StockMove, WarehouseId
from stock.services import post_sale, post_warehouse_input_to_stock


class PostSaleAutoReplenishTests(TestCase):
    def setUp(self):
    ...
    def test_auto_replenish_on_sale(self):
        move, sales_entry = post_sale(
            warehouse=self.warehouse_bar,
            lines=[{"artikl": self.artikl, "quantity": Decimal("3.0000")}],
            date=timezone.localdate(),
        ...
        transfer = StockMove.objects.filter(
            move_type=StockMove.MoveType.TRANSFER,
            from_warehouse=self.warehouse_main,
            to_warehouse=self.warehouse_bar,
        ).first()
        self.assertIsNotNone(transfer)
        self.assertIsNotNone(move.journal_entry_id)
        self.assertIsNotNone(sales_entry.id)
