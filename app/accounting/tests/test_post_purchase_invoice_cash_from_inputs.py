...
class PostPurchaseInvoiceCashFromInputsTests(TestCase):
    def setUp(self):
        ...
        artikl_5 = Artikl.objects.create(name="Test 5%", tax_group=tg_5, deposit=deposit)
        artikl_25 = Artikl.objects.create(name="Test 25%", tax_group=tg_25)

        self.wi1 = WarehouseInput.objects.create(order=po, supplier=supplier, date=date(2026, 7, 2))
        ...
        self.wi2 = WarehouseInput.objects.create(order=po, supplier=supplier, date=date(2026, 7, 3))
        WarehouseInputItem.objects.create(
    ...
    def test_post_purchase_invoice_cash_from_inputs(self):
        ...
        # expense + vat + deposit + cash
        self.assertEqual(entry.items.count(), 4)
        self.assertTrue(entry.items.filter(account=self.deposit, debit=Decimal("0.10")).exists())
        self.wi1.refresh_from_db()
        self.wi2.refresh_from_db()
        self.assertEqual(self.wi1.journal_entry_id, entry.id)
        self.assertEqual(self.wi2.journal_entry_id, entry.id)

        with self.assertRaises(Exception):
            post_purchase_invoice_cash_from_inputs(
                document_type=self.doc,
                doc_date=date(2026, 7, 4),
                inputs=[self.wi1, self.wi2],
                cash_account=self.cash,
                deposit_account=self.deposit,
            )
